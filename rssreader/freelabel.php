...
foreach ($rss->channel->item as $item) {
  ...
  $content_no_url = $item->description; 
  ...
  $hashtagAMR = urlencode('#FREELABEL');
   $feed3 .= '
   <a  href="'. $item->link .'" target="_blank"><h2 id="dash_news_feed" >' . $item->title . "</a><br>".$content_no_url."<br><input type='text' value='".$content_no_url."'><br><br>
