...
class Registration
{
    ...
    public function __construct()
    {
        ...
        // if we have such a GET request, call the verifyNewUser() method
        ...
        } elseif($_GET['resend']=='verify' && $_GET["user_id"]) {
            // send a verification email
            //echo 'okay<hr><hr><hr>okay';
            ...
            if ( $row = mysqli_fetch_array($result) ) {
               ...
               $pw_hash =  $row['user_activation_hash'];
               //echo $pw_hash. '';
               //exit;
                if ($this->sendVerificationEmail($_GET["user_id"], $_GET["user_email"], $pw_hash)) {
    ...
    private function registerNewUser($user_name, $user_email, $user_password, $user_password_repeat, $captcha)
    {
        ...
        // finally if all the above checks are ok
            ...
            $query_check_user_name = $this->db_connection->prepare('SELECT user_id,user_name, user_email,user_active FROM users WHERE user_name=:user_name OR user_email=:user_email');
            ...
            if (count($result) > 0) {
                for ($i = 0; $i < count($result); $i++) {
                    ...
                    if ($result[$i]['user_active'] == 0 ) {
                        $this->errors[] = 'This users account has not been activated, resend the email to '.$result[$i]['user_email'].'? <br><br><button onclick="window.open(\'http://freelabel.net/user/register.php?user_id='.$result[$i]['user_id'].'&user_email='.$result[$i]['user_email'].'&resend=verify\')" class="btn btn-primary" style="color:#101010;text-transform:uppercase;">Re-send Verification Email</button>';
                    }
